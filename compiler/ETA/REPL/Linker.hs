  ...
  case lookupNameEnv (closure_env pls) name of
    ...
    Nothing
        -> ASSERT2(isExternalName name, ppr name)
           do let sym_to_find = "closure"
              m <- lookupClosure hsc_env sym_to_find
              case m of
...
linkDependencies _hsc_env pls _span _needed_mods = return (pls, Succeeded)
...
linkDecls _hsc_env _span _cbc = panic "linkDecls"
  --   -- Initialise the linker (if it's not been done already)
  --   initDynLinker hsc_env
...
--             -> IO (PersistentLinkerState, SuccessFlag)
-- dynLinkObjs hsc_env pls _objs = return (pls, Succeeded)
...
-- dynLoadObjs hsc_env pls _objs = return pls
    -- let dflags = hsc_dflags hsc_env
...
--             -> IO PersistentLinkerState
-- dynLinkBCOs hsc_env pls _bcos = return pls
...
-- linkSomeBCOs _hsc_env _ce _mods = return []
-- foldr fun do_link mods []
--  where
...
linkPackage _hsc_env _pkg = return ()
   -- = do
   --      let dflags    = hsc_dflags hsc_env
