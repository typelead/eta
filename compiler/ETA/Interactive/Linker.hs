...
-- necessary.
module ETA.Interactive.Linker ( getHValue, showLinkerState,
...
import qualified ETA.Utils.Maybes as Maybes
 ...
 = do {
  ...
  where
    ...
    follow_deps :: [Module]             -- modules to follow
                ...
                -> UniqSet InstalledUnitId          -- accum. package dependencies
    ...
    follow_deps (mod:mods) acc_mods acc_pkgs
        = do
          ...
          --
          if pkg /= this_pkg
             then follow_deps mods acc_mods (addOneToUniqSet acc_pkgs' (toInstalledUnitId pkg))
...
linkDecls :: HscEnv -> SrcSpan -> ETA.Interactive.ByteCodeAsm.CompiledByteCode -> IO () --[HValue]
...
-- packages.
--
linkPackages :: DynFlags -> [LinkerUnitId] -> IO ()
...
linkPackages' :: DynFlags -> [LinkerUnitId] -> PersistentLinkerState
             -> IO PersistentLinkerState
linkPackages' dflags new_pks pls = do
  ...
  where
     ...
     link_one pkgs new_pkg
        ...
        | otherwise
        = throwGhcExceptionIO (CmdLineError ("unknown package: " ++ installedUnitIdString new_pkg))
