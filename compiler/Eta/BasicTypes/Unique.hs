...
unpkUnique (MkUnique u)
  = let
        ...
        tag = chr (u `shiftRL` 24)
        i   = u .&. 16777215
    in
    (tag, i)

shiftRL :: Int -> Int -> Int
shiftRL (I# n) (I# i) = I# (uncheckedIShiftRL# n i)
...
NOTE: Removing the Ord instance requires nontrivial changes in the
      typechecker. Keeping it for now. -RM
-}
...
************************************************************************
*                                                                      *
\subsection[Utils-base36]{Base-36 numbers}
*                                                                      *
************************************************************************

A character-stingy way to read/write numbers (notably Uniques).
The ``36-its'' are \tr{[0-9A-Z]}.  We don't handle negative Ints.
...
iToBase36 n_
  ...
  where
    go n cs
      | n < 36
      = let !c = chooseChar36 n in c : cs
      | otherwise
      = go q (c : cs)
      where (q, r) = quotRem n 36
            !c     = chooseChar36 r

    chooseChar36 :: Int -> Char
    {-# INLINE chooseChar36 #-}
    chooseChar36 (I# n) = C# (indexCharOffAddr# chars36 n)
