...
translateNPat (OverLit val False _ ty) mb_neg
  ...
  = translatePat (LitPat (HsInt i))
  | isWordTy   ty, HsIntegral i <- val
  = translatePat (mk_num_lit HsWordPrim i)
  where
    mk_num_lit c i = LitPat $ case mb_neg of
      Nothing -> c src value
      Just _  -> c src (-value)
      where src = fromMaybe "" $ il_text i
            value = il_value i
...
process_guards _us _oversimplify [] = (Singleton, Empty, Empty) -- No guard == True guard
...
-- mkPatternVarsSM tys = mapM mkPmVarSM tys
...
patVectProc oversimplify (vec,gvs) vsa = do
  us <- getUniqueSupplyM
  let (c_def, u_def, d_def) = process_guards us oversimplify gvs -- default (continuation)
...
type PmMatcher =  UniqSupply
               -> ValSetAbs
               -> Pattern
               -> PatVec    -- Vector head and tail
               -> ValAbs
               -> ValSetAbs -- VSA    head and tail
               -> ValSetAbs
