...
translateNPat :: FamInstEnvs -> HsOverLit Id -> Maybe (SyntaxExpr Id) -> PmM PatVec
translateNPat fam_insts (OverLit val False _ ty) mb_neg
  | isStringTy ty, HsIsString src s <- val, Nothing <- mb_neg
  = translatePat fam_insts (LitPat (HsString src s))
  | isIntTy    ty, HsIntegral i <- val
  = translatePat fam_insts (LitPat (HsInt i))
  | isWordTy   ty, HsIntegral i <- val
  = translatePat fam_insts (mk_num_lit HsWordPrim i)
  where
