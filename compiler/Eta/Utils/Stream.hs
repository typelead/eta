...
-- -----------------------------------------------------------------------------
{-# LANGUAGE CPP #-}
module Eta.Utils.Stream (
    Stream(..), yield, liftIO,
    collect, fromList,
    Eta.Utils.Stream.map,
    Eta.Utils.Stream.mapM,
    Eta.Utils.Stream.mapAccumL
  ) where
...
map f str = Stream $ do
   ...
   case r of
     ...
     Right (a, str') -> return (Right (f a, Eta.Utils.Stream.map f str'))
...
mapM f str = Stream $ do
   ...
   case r of
     ...
     Right (a, str') -> do
        b <- f a
        return (Right (b, Eta.Utils.Stream.mapM f str'))
