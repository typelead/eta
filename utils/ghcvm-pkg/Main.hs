...
import qualified GHCVM.PackageDb as GhcPkg
...
ourCopyright :: String
ourCopyright = "GHCVM package manager version " ++ myVersion ++ "\n"
...
usageHeader prog = substProg prog $
  ...
  "    command with --package-db={path}.  To use the new database with GHCVM,\n" ++
  "    use GHCVM's -package-db flag.\n" ++
  "\n" ++
  "  $p register {filename | -}\n" ++
  ...
  "    of the dependency graph: ghcvm-pkg dot | tred | dot -Tpdf >pkgs.pdf" ++
  "\n" ++
  ...
  "    \"ghcvm-pkg describe '*'\", except that it is intended to be used\n" ++
  ...
  "  hide, expose, and also check), ghcvm-pkg modifies the global database by\n"++
...
getPkgDatabases verbosity modify use_user use_cache expand_vars my_flags = do
  ...
  -- is done in ghcvm itself. We rely on the convention that the global
  -- package db lives in ghcvm's libdir.
  ...
  -- "ghcvm-pkg check" and all commands that modify the db) we will attempt to
  -- use the user db.
  let sys_databases
  ...
  e_pkg_path <- tryIO (System.Environment.getEnv "GHCVM_PACKAGE_PATH")
  let env_stack =
        case e_pkg_path of
       ...
       case e of
         Left err
           | ioeGetErrorType err == InappropriateType ->
              die ("ghcvm no longer supports single-file style package databases "
                ++ "(" ++ path ++ ") use 'ghcvm-pkg init' to create the database "
                  ...
                  whenReportCacheErrors $
                    if isDoesNotExistError ex
                      then do
                        ...
                        warn ("ghcvm will fail to read this package db. " ++
                              "Use 'ghcvm-pkg recache' to fix.")
                      else do
                  ...
                  if tcache >= tdir
                      then do
                          ...
                          whenReportCacheErrors $ do
                              ...
                              warn ("ghcvm will see an old view of this " ++
                                    "package db. Use 'ghcvm-pkg recache' to fix.")
...
--
-- Also perform a similar substitution for the older GHCVM-specific
-- "$topdir" variable. The "topdir" is the location of the ghcvm
...
registerPackage input verbosity my_flags auto_ghci_libs multi_instance
  ...
  when (auto_ghci_libs && verbosity >= Silent) $
    warn "Warning: --auto-ghci-libs is deprecated and will be removed in GHCVM 7.4"
  --
  s <-
    case input of
      "-" -> do
...
updateDBCache verbosity db = do
  let filename = location db </> cachefilename
      ...
      pkgsGhcvmCacheFormat :: [PackageCacheFormat]
      pkgsGhcvmCacheFormat = map convertPackageInfoToCacheFormat pkgsCabalFormat

  when (verbosity > Normal) $
      infoLn ("writing cache " ++ filename)
  GhcPkg.writePackageDb filename pkgsGhcvmCacheFormat pkgsCabalFormat
    `catchIO` \e ->
...
-----------------------------------------
-- Cut and pasted from ghcvm/compiler/main/SysTools
...
getLibDir = fmap (fmap (</> "lib")) $ getExecDir "/bin/ghcvm-pkg.exe"
...
-----------------------------------------
-- Adapted from ghcvm/compiler/utils/Panic
...
installSignalHandlers = do
  ...
  return ()
#else
  -- GHCVM 6.3+ has support for console events on Windows
