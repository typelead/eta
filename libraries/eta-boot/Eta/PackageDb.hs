...
--
-- This module provides the view of Eta's database of registered packages that
-- is shared between Eta the compiler\/library, and the eta-pkg program. It
...
-- | @eta-boot@'s copy of 'Module', i.e. what is serialized to the database.
...
-- | @eta-boot@'s copy of 'UnitId', i.e. what is serialized to the database.
...
-- | Read the part of the package DB that Eta is interested in.
--
readPackageDbForEta :: RepInstalledPackageInfo a b c d e f g =>
                       FilePath -> IO [InstalledPackageInfo a b c d e f g]
readPackageDbForEta file =
  decodeFromFile file DbOpenReadOnly getDbForEta >>= \case
    (pkgs, DbOpenReadOnly) -> return pkgs
  where
    getDbForEta = do
      _version    <- getHeader
      _etaPartLen <- get :: Get Word32
      etaPart     <- get
      -- the next part is for eta-pkg, but we stop here.
      return etaPart
...
--
readPackageDbForEtaPkg :: Binary pkgs => FilePath -> DbOpenMode mode t ->
                          IO (pkgs, DbOpenMode mode PackageDbLock)
readPackageDbForEtaPkg file mode =
    decodeFromFile file mode getDbForEtaPkg
  where
    getDbForEtaPkg = do
      _version    <- getHeader
      -- skip over the eta part
      etaPartLen  <- get :: Get Word32
      _etaPart    <- skip (fromIntegral etaPartLen)
      -- the next part is for eta-pkg
      etaPkgPart  <- get
      return etaPkgPart
...
writePackageDb :: (Binary pkgs, RepInstalledPackageInfo a b c d e f g) =>
                  ...
                  pkgs -> IO ()
writePackageDb file etaPkgs etaPkgPart =
  writeFileAtomic file (runPut putDbForEtaPkg)
  where
    putDbForEtaPkg = do
        putHeader
        put               etaPartLen
        putLazyByteString etaPart
        put               etaPkgPart
      where
        etaPartLen :: Word32
        etaPartLen = fromIntegral (BS.Lazy.length etaPart)
        etaPart    = encode etaPkgs

getHeader :: Get (Word32, Word32)
getHeader = do
    magic <- getByteString (BS.length headerMagic)
    when (magic /= headerMagic) $
      fail "not a eta-pkg db file, wrong file magic number"
    ...
    when (majorVersion /= 1) $
      fail "unsupported eta-pkg db format version"
...
headerMagic = BS.Char8.pack "\0etapkg\0"
